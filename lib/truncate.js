...
function truncMetadata (metadata, opts) {
  return breadthFilter(metadata, (value, key, path) => {
...
function truncTransaction (trans, opts) {
  const result = breadthFilter(trans, (value, key, path) => {
    if (typeof value !== 'string') {
...
function truncSpan (span, opts) {
  const result = breadthFilter(span, (value, key, path) => {
    if (typeof value !== 'string') {
      return value
    }
...
function truncError (error, opts) {
  return breadthFilter(error, (value, key, path) => {
    if (typeof value !== 'string') {
      if (isHeader(path)) return String(value)
...
function truncMetricSet (metricset, opts) {
  return breadthFilter(metricset, (value, key, path) => {
    if (typeof value !== 'string') {
      return value
    }
...
function isHeader (path) {
  return path[0] === 'context' && (path[1] === 'request' || path[1] === 'response') && path[2] === 'headers'
}
